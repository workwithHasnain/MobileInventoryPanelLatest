...
/**
 * Add a device URL to sitemap.xml
 * 
 * @param string $slug The device slug
 * @param string $lastmod The last modified date (Y-m-d format)
 * @return bool True on success, false on failure
 */
function addDeviceToSitemap($slug, $lastmod = null)
{
    global $canonicalBase;
    ...
    $device_url = $canonicalBase . '/device/' . urlencode($slug);

    // Check if URL already exists
    if (urlExistsInSitemap($device_url)) {
        return true; // URL already in sitemap, return success
    }
    ...
    $new_entry = "    <url>\n";
    $new_entry .= "        <loc>" . htmlspecialchars($device_url, ENT_XML1) . "</loc>\n";
    $new_entry .= "        <lastmod>" . $lastmod . "</lastmod>\n";
    $new_entry .= "        <changefreq>weekly</changefreq>\n";
    $new_entry .= "        <priority>0.7</priority>\n";
    $new_entry .= "    </url>\n";
...
/**
 * Remove a device URL from sitemap.xml
 * 
 * @param string $slug The device slug
 * @return bool True on success, false on failure
 */
function removeDeviceFromSitemap($slug)
{
    global $canonicalBase;
    ...
    // Find and remove the URL entry
    $pattern = '/<url>\s*<loc>' . preg_quote($device_url, '/') . '<\/loc>.*?<\/url>\s*/is';
    $updated_sitemap = preg_replace($pattern, '', $sitemap_content);
...
/**
 * Update a device URL in sitemap.xml (for slug changes)
 * 
 * @param string $old_slug The old device slug
 * @param string $new_slug The new device slug
 * @param string $lastmod The last modified date (Y-m-d format)
 * @return bool True on success, false on failure
 */
function updateDeviceInSitemap($old_slug, $new_slug, $lastmod = null)
{
    global $canonicalBase;
    ...
    $old_url = $canonicalBase . '/device/' . urlencode($old_slug);
    $new_url = $canonicalBase . '/device/' . urlencode($new_slug);
...
/**
 * Update lastmod date for a device URL in sitemap
 * 
 * @param string $slug The device slug
 * @param string $lastmod The last modified date (Y-m-d format)
 * @return bool True on success, false on failure
 */
function updateDeviceLastmodInSitemap($slug, $lastmod = null)
{
    global $canonicalBase;
    ...
    // Find and update the lastmod for this URL
    $pattern = '/<loc>' . preg_quote($device_url, '/') . '<\/loc>\s*<lastmod>.*?<\/lastmod>/i';
    $replacement = '<loc>' . htmlspecialchars($device_url, ENT_XML1) . '</loc><lastmod>' . $lastmod . '</lastmod>';
