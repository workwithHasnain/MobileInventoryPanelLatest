...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        foreach ($existing_phones as $existing_phone) {
            if (isset($existing_phone['name']) && isset($existing_phone['brand']) &&
    ...
    // If still no errors after duplicate check, create the device
    if (empty($errors)) {
        $new_phone = [
            // Launch
            ...
            'weight' => $_POST['weight'] ?? '',
            'ip_certificate' => $_POST['ip_certificate'] ?? [],
            'color' => $_POST['color'] ?? '',
            'back_material' => $_POST['back_material'] ?? '',
        ...
        if ($result) {
            ...
            // Check if it's a duplicate or system error
            $existing_phones = getAllPhones();
            $is_duplicate = false;
            foreach ($existing_phones as $existing_phone) {
                if (isset($existing_phone['name']) && isset($existing_phone['brand']) &&
            ...
            if ($is_duplicate) {
                $errors['general'] = 'A device with the exact name "' . htmlspecialchars($name) . '" by "' . htmlspecialchars($brand) . '" already exists. Please use a different model name. The same brand can have multiple different devices.';
            } else {
                $errors['general'] = 'Failed to save device data. Please check file permissions and try again.';
            }
        }
    }
}
