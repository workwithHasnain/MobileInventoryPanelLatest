...
<head>
  ...
  <link rel="canonical" href="<?php echo $canonicalBase; ?>/device/<?php echo htmlspecialchars($device_slug); ?>/images" />
  <?php
  ...
  if (!empty($device['meta_title'])) {
    $page_title = htmlspecialchars($device['meta_title']) . ' - Images & Gallery';
  } else {
    $page_title = htmlspecialchars($brand_name . ' ' . $device_name) . ' - Official Images, Photos & Gallery | DevicesArena';
  }

  // Use custom meta description if available, otherwise auto-generate from specs
  if (!empty($device['meta_desc'])) {
    $meta_description = htmlspecialchars($device['meta_desc']) . " - High resolution images gallery.";
  } else {
    $meta_description = htmlspecialchars("Explore official high-resolution images, design photos, and look into the gallery of the {$brand_name} {$device_name} on DevicesArena.");
  }
  ?>
      ...
      <main>
        ...
        <div class="da-device-hero">
          ...
          <div class="da-device-info">
            <!-- Brand label -->
            <div class="da-device-brand-label">
              <span class="da-brand-dot"></span>
              <?php echo htmlspecialchars($device['brand_name'] ?? 'Device'); ?>
            </div>
        ...
        <div class="da-widget mt-4">
          <div class="da-widget-header">
            <h3>Official Images & Gallery</h3>
            <div class="da-widget-icon"><i class="fa fa-images"></i></div>
          </div>
          <div class="da-widget-body p-4">
            <?php if (!empty($deviceImages)): ?>
              <div class="da-device-images-stack" style="display: flex; flex-direction: column; gap: 32px;">
                <?php foreach ($deviceImages as $index => $image): ?>
                  <div class="da-device-image-card" style="background: var(--bg-card); border: 1px solid var(--border); border-radius: var(--radius); padding: 32px 16px; display: flex; justify-content: center; align-items: center; transition: var(--transition); position: relative; overflow: hidden;">
                    <img src="<?php echo htmlspecialchars(getAbsoluteImagePath($image, $base)); ?>" 
                         alt="<?php echo htmlspecialchars(($device['brand_name'] ?? '') . ' ' . ($device['name'] ?? 'Device')); ?> - Image <?php echo $index + 1; ?>"
                         style="max-width: 100%; height: auto; object-fit: contain; max-height: 800px; filter: drop-shadow(0 8px 24px rgba(0, 0, 0, 0.2)); transition: transform 0.3s cubic-bezier(0.2, 0, 0, 1);"
                         class="img-fluid da-gallery-img"
            ...
            <?php else: ?>
              <div class="text-center py-5" style="color: var(--text-muted);">
                ...
                <p class="mb-0 fw-medium">No additional images available for this device.</p>
              </div>
            <?php endif; ?>
          </div>
        </div>
