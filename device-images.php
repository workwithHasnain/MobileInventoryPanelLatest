  ...
  <div class="container my-2" style="
    ...
    <div class="row">
      <div class="col-lg-8 col-md-7 order-1" style="padding-right: 0;">
        <div class="bg-white">
          <div class="px-3 py-2">
            <h4 style="font-family: system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Oxygen, Ubuntu, Cantarell, 'Open Sans', 'Helvetica Neue'; font-weight: 600; color: #1B2035; border-bottom: 2px solid #D7CCC8; padding-bottom: 10px; margin-bottom: 20px;">
              <?php echo htmlspecialchars(($device['brand_name'] ?? '') . ' ' . ($device['name'] ?? 'Device')); ?> Images
            </h4>
            
            <?php if (!empty($deviceImages)): ?>
              <div class="device-images-container">
                <?php foreach ($deviceImages as $index => $image): ?>
                  <div class="mb-4 text-center" style="background: #F5F5F5; padding: 20px; border-radius: 8px; border: 1px solid #EFEBE9;">
                    <img src="<?php echo htmlspecialchars($image); ?>" 
                         alt="<?php echo htmlspecialchars(($device['brand_name'] ?? '') . ' ' . ($device['name'] ?? 'Device')); ?> - Image <?php echo $index + 1; ?>"
                         style="max-width: 100%; height: auto; object-fit: contain; max-height: 800px; display: inline-block;"
