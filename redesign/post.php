...
<body>
  ...
  <div class="da-page">
    <div class="da-content-area">
      ...
      <main>
<!-- Post Hero -->
<section class="da-device-hero mb-5">
  <div class="da-device-img" onclick="window.location.href='<?= $base ?>post/<?= htmlspecialchars($slug) ?>/images'" title="View all images">
    <?php if (!empty($post['featured_image'])): ?>
      <img src="<?= htmlspecialchars(getAbsoluteImagePath($post['featured_image'], $base)) ?>" alt="<?= htmlspecialchars($post['title']) ?>">
      <div class="da-device-img-overlay"><i class="fa fa-images"></i> <span>Gallery</span></div>
    <?php else: ?>
      <div class="da-img-fallback"><i class="fa fa-file-alt"></i></div>
    <?php endif; ?>
  </div>
  ...
  <div class="da-device-info">
    <div class="da-device-brand-label">
      ...
      <?= htmlspecialchars($post['author_name'] ?? 'Author') ?>
    </div>
    <h1 class="da-device-title"><?= htmlspecialchars($post['title']) ?></h1>
    <div class="da-hero-mid-row">
      <div class="da-highlights-col">
        <?php if (!empty($post['meta_description'])): ?>
          <div class="da-highlight-chip">
            <div class="da-highlight-chip-text">
              <span class="da-highlight-chip-val"><?= htmlspecialchars($post['meta_description']) ?></span>
            </div>
          </div>
        <?php endif; ?>
      </div>
      <div class="da-cta-col">
        <span class="badge bg-primary ms-2"><?= $postCommentCount ?> comments</span>
      </div>
    </div>
  </div>
</section>
