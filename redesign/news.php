...
// Posts — news only
$posts_stmt = $pdo->prepare("SELECT p.*,(SELECT COUNT(*) FROM post_comments pc WHERE pc.post_id=p.id AND pc.status='approved') as comment_count FROM posts p WHERE p.status ILIKE 'published' AND p.is_news = true ORDER BY p.created_at DESC LIMIT 20");
$posts_stmt->execute();
  ...
  <div class="da-page">
    ...
    <section class="da-hero" aria-label="Featured News">
      ...
      <div class="da-hero-left">
        <div class="da-section-label"><span>News</span></div>
      ...
      <main>
        <div class="da-post-feed-header">
          <div>
            ...
            <h2 class="da-section-title">Latest News</h2>
          </div>
          <a href="<?php echo $base; ?>news" class="da-view-all">View All <i class="fa fa-arrow-right"></i></a>
        </div>
  ...
  <script>
    ...
    (function() {
      ...
      function loadMore() {
        ...
        fetch(baseURL + 'load_posts.php?page=' + page + '&type=news&format=block')
          .then(r => r.json())
          .then(data => {
